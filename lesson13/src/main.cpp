...
void test1() {
    ...
    double med;
    {
        ...
        for (auto &i: matches01) {
            distances.push_back(i[0].distance);
            distances.push_back(i[1].distance);
        }
        std::sort(distances.begin(), distances.end());
        std::cout << "matches01 distances min/median/max: " << distances[0] << "/" << distances[distances.size() / 2]
                  << "/" << distances[distances.size() - 1] << std::endl;
        med = distances[distances.size() / 2];
    }
    for (int k = 0; k < 2; ++k) {
        std::vector<cv::DMatch> matchesK;
        matchesK.reserve(matches01.size());
        for (auto &i: matches01) {
            matchesK.push_back(i[k]);
        }
    ...
    {
        std::vector<double> distances;
        for (const auto &i: matches10) {
            distances.push_back(i[0].distance);
            distances.push_back(i[1].distance);
        }
        std::sort(distances.begin(), distances.end());
        std::cout << "matches10 distances min/median/max: " << distances[0] << "/" << distances[distances.size() / 2]
                  << "/" << distances[distances.size() - 1] << std::endl;
    }
    for (int k = 0; k < 2; ++k) {
        std::vector<cv::DMatch> matchesK;
        matchesK.reserve(matches10.size());
...
//
        ...
        if (match01.queryIdx != match10.trainIdx) {
            isOk = false;
        }
        ...
        // K-RATIO отсекает порядка 90% сопоставлений
        // Проверка расстояния на величину при хорошем подборе констант отсекает до 80 процентов сопоставлений
        // Их использование вместе не особо меняет картину, 90% K-RATIO не удается превзойти
        // С Left - Right check ситуация как и с проверкой расстояние
        // В целом, комбинация всех методов дает лучший результат, но не намного, чем каждый из методов в отдельности
        // Также для корректного постороения матрицы гомографии далее часто хватает и одного любого из этих методов(RANSAC справляется)
        // RANSAC'у приходится отсекать меньше процентов выбросов
    ...
    std::cout << H01 << std::endl;
    {
        ...
        for (int i = 0; i < inliersMask.size(); ++i) {
            if (inliersMask[i]) {
                ...
                inlierKeypoints0.emplace_back(pointsGood0[i], 3.0);
                inlierKeypoints1.emplace_back(pointsGood1[i], 3.0);
            }
        }
